...
function parseDate(value) {
  ...
  const formats = ["YYYY-MM-DD", "DD/MM/YYYY", "D/M/YYYY", "DD/MM/YY", "D/M/YY"];
...
function resolveFile() {
  const p1 = path.resolve("data", "GA_calidad_2010-2013.xlsx");
  const p2 = path.resolve("data", "GA_calidad_2010-2013_.xlsx");

  if (fs.existsSync(p1)) return p1;
  if (fs.existsSync(p2)) return p2;

  throw new Error("No se encontró GA_calidad_2010-2013.xlsx");
}
...
export async function run() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      await withTx(async (client) => {
        ...
        const anioNum = toNum(r["ANHO"]);
        const anio = Number.isFinite(anioNum) ? Math.trunc(anioNum) : 2010;
        const fecha =
          parseDate(r["Fecha_mues"]) ??
          `${anio}-01-01`;
  ...
  console.log("Importación 2010-2013 finalizada");
...
if (process.argv[1].endsWith("p2010_2013.js")) {
  run().catch((e) => {
    console.error(e);
    process.exit(1);
  });
